<head>
  ...
  function createNewElementProd() {
    ...
    newProdElement.style =
      "overflow:visible ! important; background-color:antiquewhite;min-height:auto !important; position:static; border:2px solid gold; margin-bottom:4rem;";
    newProdElement.innerHTML = `
        <form style="display:flex;" action="{{ route('headerProducts.store') }}" method="POST">
          ...
          <button class="cancel delete" type="button" style="height:100%;" onclick="cancelNewElementProd(this)">Cancel</button>
          <button class="Create save" style="height: 4rem;" type="submit">Save New element</button>
        </form>
      `;

      headerContainer.insertBefore(newProdElement, headerContainer.firstChild);
  }
    function cancelNewElementProd(cancelButton) {
      const newProdElement = cancelButton.closest('.headerProd');
      newProdElement.remove();
    }
  </script>
</head>
...
<section id="headerPr">
  <header style="display:flex;flex-direction:column;border:2px solid cornflowerblue;margin:2rem;" class="hp">
    <h2 style="margin:1rem auto 2rem 0;font-size:2rem;">Header Products</h2>
    <button class="create" onclick="createNewElementProd()">Create <i class="fas fa-plus"></i></button>
