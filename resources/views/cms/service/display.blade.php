<head>
  ...
  function createNewElementService() {
    ...
    newServiElement.style =
      "border: 1px solid orange; background: antiquewhite; margin-bottom: 2rem;";
    ...
    newServiElement.innerHTML = `
        <form action="{{ route('service.store') }}" method="POST">
          ...
          <div class="box cmsBoxDisplay">
            ...
            <input style="display: block;" type="text" name="icon" value="">
            <div>
              <h3 contenteditable="true" oninput="this.nextElementSibling.value = this.textContent;">Service Title</h3>
              <input type="hidden" name="title" value="">
              <p contenteditable="true" oninput="this.nextElementSibling.value = this.textContent;">Service Description</p>
    ...
    servicesContainer.insertBefore(newServiElement, createButton);
  }
  </script>
</head>
...
<section id="services" style="border:2px solid cornflowerblue;margin:2rem;">
  ...
  <div id="allServices">
    ...
    <div class="ser" style="border:1px solid orange;background:antiquewhite;margin-bottom:2rem;">
      <form action="{{ route('service.update', $service->id) }}" method="POST" data-id="{{ $service->id }}">
        ...
        <div class="box cmsBoxDisplay">
          ...
          <input style="display:block;" type="text" name="icon" value="{{ $service->icon }}">
          <div>
            <h3 contenteditable="true" oninput="this.nextElementSibling.value = this.textContent;">{{ $service->title }}
            </h3>
    ...
    <button class="create" onclick="createNewElementService()">Click to Create a entirely new Service elememt.</button>
  </div>
</section>
