...
<section id="recentWorks">
  <h2>Recent Works</h2>
  <button class="create" id="createRe" onclick="createNewElementRecentWork()"
    style="height:100%;">Create <i class="fas fa-plus"></i></button>
  <div id="honey">
    @foreach($recentWorks as $recentWork)
    <form action="{{ route('recentWorks.update', $recentWork->id) }}" method="POST" enctype="multipart/form-data"
      data-id="{{ $recentWork->id }}" style="border:2px solid gold;background-color:antiquewhite;border-radius:1vw;">
      @csrf
      @method('PUT')
      <div class="rwel">
        <div class="rw">
          <div class="imgL">
            <label for="imageInput{{ $recentWork->id }}">
              <img id="preview{{ $recentWork->id }}" src="/images/recentWorks/{{ $recentWork->imgsrc }}"
                alt="{{ $recentWork->imgalt }}" width="100">
            </label>
            ...
            <span contenteditable="true"
              oninput="updateHiddenInputValue(this, 'titleA')">{{ $recentWork->titleA }}</span>
            <span style="font-weight:100;font-size:1rem;" contenteditable="true"
              oninput="updateHiddenInputValue(this, 'titleB')">{{ $recentWork->titleB }}</span>
          </div>
          <div class="tec" contenteditable="true" oninput="updateHiddenInputValue(this, 'description')">
            {{ $recentWork->description }}</div>
        </div>
...
function createNewElementRecentWork() {
  ...
  const createButtonRe = document.querySelector('#createRe');
  const parentElementRe = createButtonRe.parentNode;
  const newRecentWorkElement = document.createElement('div');
  newRecentWorkElement.className = 'rwel';
  newRecentWorkElement.style = "width:25%;border:2px solid gold;background-color:antiquewhite;border-radius:1vw;";

  newRecentWorkElement.innerHTML = `
    <form action="{{ route('recentWorks.store') }}" method="POST" enctype="multipart/form-data">
      @csrf
      <div class="rw">
        <div class="imgL">
          <label for="imageInput{{ $recentWork->id }}">
            <img id="preview{{ $recentWork->id }}" src="/images/recentWorks/{{ $recentWork->imgsrc }}" alt="{{ $recentWork->imgalt }}" width="100">
          </label>
          <input type="file" id="imageInput{{ $recentWork->id }}" name="image" accept="image/*" style="display:none;">
      ...
      <div style="display:inline-block;margin-top:auto;">
        <button class="cancel delete" type="button" onclick="cancelNewElementRecent(this)">Cancel</button>
        <button style="margin-top:1rem;" class="save" type="submit">Save New Element</button>
      </div>
    </form>
  `;

  parentElementRe.insertBefore(newRecentWorkElement, createButtonRe.nextSibling);
}

function cancelNewElementRecent(cancelButton) {
  const newRwelElement = cancelButton.closest('.rwel');
  newRwelElement.remove();
}
