...
<section id="headerSe">
  <header style="display:flex;flex-direction:column;border:2px solid cornflowerblue;margin:2rem;" class="hs">
    <h2 style="margin:1rem auto 2rem 0;">header services section</h2>
    ...
    <div class="headerMain headerServ"
      ...
      <form style="display:flex;" action="{{ route('headerServices.update', $headerService->id) }}" method="POST"
        ...
        <div class="hMB hmbActive">
          <div class="hMBFirst">
            <p contenteditable="true" oninput="updateHiddenInputValue(this, 'title')">{{ $headerService->title }} <i
          ...
          <div class="hMBTexts">
            ...
            @endforeach
            <button class="add-link" type="button" onclick="addSubTitleAndTextServ(this)">Add more sub titles and
              texts</button>
          </div> <!-- .hMBTexts -->
        </div> <!-- .hMB -->
    ...
    <button class="create" onclick="createNewElementServ()">Click to Create a entirely new headerService elememt.</button>
  </header>
</section>
